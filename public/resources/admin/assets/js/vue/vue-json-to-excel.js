Vue.component('downloadExcel', {
    template: '<span :id="idName" @click="generate"> <slot></slot></span>',
    props: {
        // mime type [xls, csv]
        type: {
            type: String,
    ...
    computed: {
        ...
        downloadFields() {
            if (this.cFields !== undefined) return this.cFields;
    ...
    methods: {
        async generate() {
            ...
            if (typeof this.fetch === 'function' || !data){
                if(this.variableplus != ''){
                    data = await this.fetch(this.variableplus);
                }else{
